...
<section>
	<article>
		...
		<table style="display: flex; flex-direction:column;">
			<c:forEach items="${serviceRunList}" var="srl" varStatus="loop">
				<tr class="fnq-trow" onClick="slideToggleFuc(${loop.index});" id="toggleClass" style="cursor:pointer;">
					<td class="fnq-tcol" style="color:gray; font-weight:bold; padding: 10px;">${srl.inquirylist}</td>
					<td class="fnq-tcol" style="color:gray; font-weight:bold; padding: 10px;">${srl.subject}</td>
				...
				<tr class="faq-content" style="display:none;">
				 	<td class="faq-tcol" style="display:block; text-align:center;">${srl.subject}</td>
            		<td class="faq-content-tcol" colspan="3" style="display:block; text-align:center;"><pre>${srl.content}</pre></td>
       			</tr>
			</c:forEach>
		</table>
		
		<jsp:include page="../paging/serviceRunPaging.jsp">
			<jsp:param value="servicerun?inquirylist=서비스/이용" name="address"/>
		</jsp:include>
